...
## 1. Introduction
### 1.1 Purpose
Define functional and non-functional requirements for the Robot Management Console, covering both backend API and frontend UI necessary to supervise and teleoperate `HACK_<n>` robots.
...
- **FR-FLEET-2**: Frontend shall display robots using names that match `HACK_<n>` where `1 ≤ n ≤ 10000` (derive ID if backend data is missing).
- **FR-FLEET-3**: UI shall show availability badge (`Online · <substate>` or `Offline · <substate>`) based on backend `status`/`subStatus` fields, falling back to signal/battery heuristics only when absent.
...
### 3.4 Teleop Tab
- **FR-TELE-1**: Manipulator card shall provide range sliders for shoulder (-30°..150°), elbow (0°..135°), wrist (-90°..90°), and grip (0–100%).
- **FR-TELE-2**: Hand preview shall render shoulder/elbow/wrist segments plus five articulated fingers that update live with slider/grip input.
- **FR-TELE-3**: Clicking "Send joint update" shall POST `/api/robots/:id/commands` with `{ type: 'manipulator', value: 'joint-update', metadata: { shoulder, elbow, wrist, grip } }` and display success/error message.
- **FR-TELE-4**: Grip hold/release quick actions shall clamp the grip state to fully closed/open and refresh the preview.
